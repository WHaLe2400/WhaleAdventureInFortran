...
program test_lu_decomposition
    ...
    integer(8) :: iostat_1, iostat_2, arg_count
    ...
    if (arg_count < 4) then
        write(*,*) "错误：参数不足。"
        write(*,*) "用法: ./DicentVersion --in_file <输入文件路径> --out_file <输出文件路径>"
        stop 1
    end if

    do k = 1, arg_count, 2
        call get_command_argument(k, arg_value)
        if (trim(arg_value) == "--in_file") then
            call get_command_argument(k+1, INPUT_PATH)
        else if (trim(arg_value) == "--out_file") then
            call get_command_argument(k+1, OUTPUT_PATH)
        else
            write(*,*) "错误：未知参数 ", trim(arg_value)
            stop 1
        end if
    end do
